#!/usr/bin/env python3
"""
Script para reentrenar el modelo ML con versiones modernas de NumPy y scikit-learn
"""
    ...
    for img_file in glob.glob(os.path.join(images_dir, "*.jpg")):
        ...
        for i, (pixel, label) in enumerate(zip(pixels, mask_flat)):
            if label > 0:  # Solo píxeles anotados
                ...
                r, g, b = pixel.astype(np.float64)
                features = [
                    r, g, b,  # RGB
                    (r + g + b) / 3,  # Intensidad promedio
                    max(r, g, b) - min(r, g, b),  # Contraste
                    r - g,  # Diferencia R-G
                    g - b,  # Diferencia G-B
                    r / (g + b + 1),  # Ratio R/(G+B)
                    g / (r + b + 1),  # Ratio G/(R+B)
...
def train_model(X, y):
    ...
    with open("modelo_perfeccionado.pkl", "wb") as f:
        pickle.dump(model_data, f)
    
    print("OK: Modelo reentrenado y guardado como 'modelo_perfeccionado.pkl'")
    return model, scaler, encoder

def main():
    """Función principal"""
    print("Reentrenando modelo ML con versiones modernas...")
    ...
    print("EXITO: Modelo reentrenado exitosamente!")
